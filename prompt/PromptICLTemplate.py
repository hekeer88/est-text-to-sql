...
class BasicICLPrompt(object):
    ...
    def format(self, target: dict, max_seq_len: int, max_ans_len: int, scope_factor: int, cross_domain=False, *args, **kwargs):
        ...
        translation_example = (
            "/* Translation example: */\n"
            "/* Question: Kui palju inimesi elab linnas? */\n"
            "/* Translate into English: How many people live in the city? */\n\n"
            )
        ...
        if self.NUM_EXAMPLE != 0:
            ...
            example_prefix = translation_example + self.get_example_prefix()
            selected_examples = []
            for example in examples:
